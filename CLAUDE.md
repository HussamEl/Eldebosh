...
| الملف | ما هو | يُحدَّث |
|---|---|---|
| `STATE.md` | لوحة الحالة | **آلياً** — `npm run state` |
| `DECISIONS.md` | سجل القرارات المغلقة | يدوياً عند قرار جديد |
| `ISSUES.md` | المشاكل بأدلتها | يدوياً عند ظهور أو حل |
| `CHANGELOG.md` | سجل التغييرات وبروتوكول التسليم | سطران عند كل تسليم |
| `RISKS.md` | سجل المخاطر | عند تغيّر خطر |
| حقل `stage` | خط إنتاج المحتوى | مع كل صفحة |

**عند فتح أي محادثة:** اقرأ `STATE.md` أولاً — سطر واحد يغني عن شرح طويل.
...
التفاصيل في `PHOTO_NAMING.md`.

## نظام المراجع
**لا تطلب صورة أُرسلت من قبل.** كل صورة لها رمز في `ASSETS.md` ومسار في المستودع.
```
npm run assets     يعيد توليد الفهرس
```
**القاعدة العامة:** إن احتجت معلومة من محادثة سابقة فهي ناقصة من الوثائق — أضفها بدل البحث عنها. التفاصيل في `REFERENCE_SYSTEM.md`.
...
- **شريط التصفية زران فقط:** `Alla` و`Vi använder själva`. لا تُضف فلاتر بلا طلب صريح — كثرة الخيارات تشتّت.
...
## الرفع اليدوي
**احذف كل محتويات المجلد العام قبل الرفع.** الرفع فوق القديم يترك صفحات لم تعد تُولَّد، فتظهر بتصميم قديم ويصل إليها الزائر. حدث هذا ثلاث مرات في مرحلة التحضير. التفاصيل في `UPLOAD.md`.
