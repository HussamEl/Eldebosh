...
for (const file of list) {
  ...
  // 11. الهيكل لا يبيع — `I-026`
  //
  // `INSTRUCTIONS` ٦٫٤: الهيكل المنشور «لا يبيع شيئاً: لا بطاقة منتج، ولا
  // مقارنة، ولا رابط أفلييت — وبالتالي لا إفصاح تجاري، فالإفصاح على صفحة بلا
  // روابط يربك القارئ ويُضعف الإفصاح في كل مكان».
  //
  // وقالب الأدلة كان يعكسها تماماً: يحجب الإفصاح ويُبقي بطاقتي الشراء. فصفحة
  // تقول «لم تُكتب بعد» كانت تبيع، وتبيع بلا إفصاح — وهو تعرّض قانوني في
  // السويد لا مجرد خطأ تحريري. لم يظهر قبل `2026-09-20` لأن أول دليل بترشيحات
  // وبمرحلة `written` هو أول ما أصاب هذا الباب.
  if (d.querySelector('.notwritten')) {
    if (d.querySelector('[rel~="sponsored"], [data-affiliate]')) {
      note(short, 'هيكل منشور يعرض رابط شراء — الهيكل لا يبيع (INSTRUCTIONS ٦٫٤)');
    }
    if (d.querySelector('.compare-table, .tile-cta')) {
      note(short, 'هيكل منشور يعرض جدول مقارنة أو زرّ شراء');
    }
  }
