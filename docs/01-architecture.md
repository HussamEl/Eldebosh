...
`site/` **يُتابَع في Git** رغم كونه ناتج بناء — وهذا مقصود لا إهمال: وظيفة النشر
في `verify.yml` **لا تبني شيئًا**، بل تنسخ `site/` المدفوع إلى فرع `deploy` الذي
تسحب منه الاستضافة. فالمدفوع هو المنشور حرفيًا.

وما يجعل ذلك آمنًا حارسٌ واحد: `npm run check:drift` يبني من المصدر ويقارن، ويرفض
أي `site/` لا يطابق بناءً نظيفًا — فلا يمكن أن يُنشر ناتجٌ لا يقابله مصدر.
